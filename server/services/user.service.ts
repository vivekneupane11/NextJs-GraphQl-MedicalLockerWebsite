...
async function findAll(): Promise<Array<User> | null> {
...
export default class UserService {
  async signUpUser(input: Partial<User>) {
    try {
      ...
      await disconnectDB();
      sendMail(user.email, input.password);
  ...
  async UpdateCaregiver(input: Partial<User>) {
    try {
  ...
  async UpdateUser(input: Partial<User>) {
    try {
      const count = await UserModel.count();
      if (count === 0) {
        input.role = "superadmin";
      }
      const userExist = await UserModel.findOneAndDelete({
        email: input.email,
      });
  ...
  async getCaregiver(id: string, { req, res }: Context) {
    try {
      // const post = await ServiceModal.findById(id).populate('user').lean();
      const user = await UserModel.findById(id);
      if (!user) return new ValidationError("No caregiver with that id exists");
